...
/**
 * Auto-refresh prices on all active shopping lists after a receipt is confirmed.
 * Only updates lists at the same store as the receipt.
 *
 * Called automatically after receipt confirmation to ensure active lists
 * reflect the latest learned prices.
 *
 * Returns the total count of items updated across all lists.
 */
export const refreshActiveListsFromReceipt = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Build receipt items for matching
    ...
    // Find user's active + shopping lists for the same store
    ...
    for (const list of sameStoreLists) {
      ...
      // Build candidates from list items
      const candidates: CandidateItem[] = items
        .filter((item) => !item.isChecked && !item.priceOverride)
        .map((item) => ({
          id: item._id,
      ...
      for (const result of matched) {
        if (result.bestMatch && result.matchScore >= 70) {
          ...
          if (listItem && result.receiptItem.unitPrice !== listItem.estimatedPrice) {
            await ctx.db.patch(result.bestMatch.id as any, {
      ...
      // Also try direct fuzzy matching for items that didn't match via multi-signal
      // (backwards compatibility + catch edge cases)
      for (const item of items) {
        if (item.isChecked || item.priceOverride) continue;

        // Skip if already updated via multi-signal
        const alreadyMatched = matched.some(
          (m) => m.bestMatch?.id === item._id
        );
        ...
        if (learnedMatch && learnedMatch.confidence >= 60) {
          // Find receipt item matching the canonical name
          const receiptMatch = receiptItemsForMatching.find((ri) =>
            isDuplicateItemName(learnedMatch.canonicalName, ri.name) ||
            isDuplicateItemName(item.name, ri.name)
          );

          if (receiptMatch && receiptMatch.unitPrice !== item.estimatedPrice) {
            await ctx.db.patch(item._id, {
              estimatedPrice: receiptMatch.unitPrice,
              priceSource: "personal",
              priceConfidence: 0.9 * (learnedMatch.confidence / 100),
              updatedAt: now,
            });
            listItemsUpdated++;
            totalUpdated++;
        ...
        // Fallback: direct fuzzy matching
        const matchingReceiptItem = receiptItemsForMatching.find((ri) =>
          isDuplicateItemName(item.name, ri.name)
        );

        if (matchingReceiptItem && matchingReceiptItem.unitPrice !== item.estimatedPrice) {
          await ctx.db.patch(item._id, {
            estimatedPrice: matchingReceiptItem.unitPrice,
            priceSource: "personal",
      ...
      if (listItemsUpdated > 0) {
        // Update list timestamp to signal change
