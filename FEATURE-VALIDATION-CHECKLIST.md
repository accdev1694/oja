...
| 15 | Onboarding (Welcome flow, cuisine, store, pantry seeding) | :white_check_mark: Done | 2026-04-08 | 45+ issues fixed (9C/12H/6M/6L + 18 audit): completeOnboarding idempotency, free-tier bulkCreate cap, AI token limit 4k→16k, prompt sanitization, stripCodeBlocks CRLF+regex, safeHaptics (welcome/admin-setup/_layout), useRef double-gen guard, O(n²)→O(1) itemIndexMap, reactive name pre-fill, parseSeedResponse schema validation+type narrowing, server-side input validation, admin query skip guards, redirect race fixes, empty states, error alerts, layout screen declarations, variable shadowing, fallback unit consistency |
